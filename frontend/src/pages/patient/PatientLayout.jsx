...
function SidebarContent({ user, initials, savedAvatar, getBadge, onLogout, onClose }) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div style={{ height: 3, background: 'linear-gradient(90deg, #0f172a 0%, #0d9488 50%, #0284c7 100%)', flexShrink: 0 }} />

            {/* Brand */}
            <div style={{ padding: '16px 14px 12px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                <Logo size={34} />
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', lineHeight: 1.1, letterSpacing: '-0.02em' }}>Priyansh Care</div>
                    <div style={{ fontSize: 9, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.1em', marginTop: 2, fontWeight: 600 }}>Patient Portal</div>
                </div>
            </div>

            {/* User card */}
            <div style={{ margin: '10px 10px 0', padding: '10px 12px', borderRadius: 10, background: 'linear-gradient(135deg, #f0fdf4 0%, #f8fafc 100%)', border: '1px solid #d1fae5', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                {savedAvatar ? (
                    <img src={savedAvatar} alt="av" style={{ width: 34, height: 34, borderRadius: 9, objectFit: 'cover', border: '2px solid #0d9488', flexShrink: 0 }} />
                ) : (
                    <div style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg, #0f172a, #0d9488)', color: '#fff', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(13,148,136,.25)' }}>
                        {initials}
                    </div>
                )}
                <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#0f172a', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.sub || 'Patient'}</div>
                    <div style={{ fontSize: 10, color: '#0d9488', fontWeight: 600, marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                            ...
                            return (
                                <NavLink
                                    ...
                                    style={({ isActive }) => ({
                                        ...
                                        background: isActive ? 'linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%)' : 'transparent',
                                        textDecoration: 'none', marginBottom: 2,
                                        borderLeft: isActive ? '3px solid #0d9488' : '3px solid transparent',
                                        transition: 'all .15s ease',
                                        boxShadow: isActive ? '0 1px 4px rgba(13,148,136,.1)' : 'none',
                                    })}
                                    onMouseEnter={e => { if (!e.currentTarget.getAttribute('data-active')) { e.currentTarget.style.background = '#f8fafc'; }}}
                                ...
                                >
                                    ...
                                    {badge && (
                                        <span style={{ background: item.badge === 'bills' ? '#fee2e2' : '#0d9488', color: item.badge === 'bills' ? '#dc2626' : '#fff', fontSize: 9, padding: '2px 6px', borderRadius: 6, fontWeight: 700 }}>
                                            {badge}
                                        </span>
                                    )}
                                </NavLink>
...
export default function PatientLayout({ upcomingCount = 0, unpaidCount = 0 }) {
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f1f5f9', fontFamily: "'Inter', system-ui, sans-serif" }}>
            ...
            {/* Desktop sidebar */}
            {!isMobile && (
                <div style={{ width: 228, background: '#fff', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100vh', overflowY: 'auto', boxShadow: '4px 0 20px rgba(0,0,0,.04)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', background: '#fff', borderBottom: '1px solid #e2e8f0', gap: 12, flexShrink: 0, boxShadow: '0 1px 10px rgba(0,0,0,.05)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 9, flex: 1 }}>
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: 9, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.08em' }}>Patient Portal</div>
                            </div>
                        </div>
                        {savedAvatar ? (
                            <img src={savedAvatar} alt="av" style={{ width: 30, height: 30, borderRadius: 8, objectFit: 'cover', border: '2px solid #0d9488', flexShrink: 0 }} />
                        ) : (
                            <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg, #0f172a, #0d9488)', color: '#fff', fontSize: 11, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                {initials}
                            </div>
                        )}
                    </div>
