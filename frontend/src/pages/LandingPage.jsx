...
function friendlySignupError(err) {
    ...
    const isDupe =
        lower.includes('duplicate') || lower.includes('duplicate entry') ||
        lower.includes('duplicate key') || lower.includes('already exists') ||
        lower.includes('already registered') || lower.includes('already in use') ||
        lower.includes('unique constraint') || lower.includes('violates unique') ||
        lower.includes('integrity constraint') || lower.includes('constraint violation') ||
        lower.includes('could not execute') || lower.includes('uk_') ||
        lower.includes('uq_') || lower.includes('unique index') || lower.includes('constraint [');

    if (status === 409) {
        if (isPhone) return { field: 'phone',    msg: 'This phone number is already registered.', action: { label: 'Try with Different Number', type: 'retry' } };
        return             { field: 'username',  msg: 'An account with this email already exists.', action: { label: 'Try with Different Email', type: 'retry' } };
    }
    if (isDupe) {
        if (isPhone) return { field: 'phone',    msg: 'This phone number is already registered.', action: { label: 'Try with Different Number', type: 'retry' } };
        if (isEmail) return { field: 'username', msg: 'An account with this email already exists.', action: { label: 'Try with Different Email', type: 'retry' } };
        return             { field: 'username',  msg: 'An account with these details already exists.', action: { label: 'Try with Different Email', type: 'retry' } };
    }
    if (status === 400) {
...
function FieldErr({ msg, action, onAction }) {
    ...
    return (
        <div className="field-err">
            ⚠ {msg}
            {action && (
                <span className="field-err-action" onClick={onAction} role="button" tabIndex={0}
        ...
        <div className="modal-body">
            ...
            <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div className="form-group">
                    ...
                    <PasswordInput id="login-pass" placeholder="Enter your password"
            ...
            <div className="trust-badges">
                {['🔒 Secure Login', 'P Priyansh Care', '✅ Verified'].map(b => (
...
function SignupModal({ onClose, onSwitchToLogin }) {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        setGlobalError(''); setGlobalAction(null); setFieldActions({});
        const errs = validate();
        setFieldErrors(errs);
            ...
            <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div className="form-row-2">
                    <div className="form-group">
                        ...
                        <input id="fullName" type="text" placeholder="Priyanshu Jaiswal"
                    ...
                    <div className="form-group">
                        ...
                        <FieldErr msg={fieldErrors.phone} action={fieldActions.phone}
                                  onAction={() => { update('phone', ''); setFieldActions(p => { const n={...p}; delete n.phone; return n; }); }}/>
                        {fieldActions.phone?.type === 'retry' && (
                            <div className="dupe-callout dupe-callout--phone">
                                <span>📱</span>
                                <span>This number is already registered.{' '}
                                    <strong className="dupe-link" onClick={() => { update('phone', ''); setFieldActions(p => { const n={...p}; delete n.phone; return n; }); }}>
                ...
                <div className="form-group">
                    ...
                    <FieldErr msg={fieldErrors.username} action={fieldActions.username}
                              onAction={() => {
                                  if (fieldActions.username?.type === 'retry') {
                                      update('username', ''); setFieldActions(p => { const n={...p}; delete n.username; return n; });
                                  } else { switchToLogin(form.username.trim()); }
                              }}/>
                        ...
                        <div className="dupe-callout dupe-callout--email">
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ marginTop: '4px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                                    <strong className="dupe-link" onClick={() => { update('username', ''); setFieldActions(p => { const n={...p}; delete n.username; return n; }); }}>
                                        Try with Different Email →
                                    </strong>
                                    <span className="dupe-link-secondary" onClick={() => switchToLogin(form.username.trim())}>
                ...
                <div className="form-group">
                    ...
                    <PasswordInput id="password" placeholder="Min 6 characters"
                ...
                <div className="form-group">
                    ...
                    <PasswordInput id="confirm" placeholder="Repeat your password"
...
export default function LandingPage() {
    ...
    function handleSwitchToLogin(email = '') { setLoginPrefillEmail(email); setModal('login'); }
    ...
    return (
        <div style={{ fontFamily: "'DM Sans','Outfit',sans-serif", background: '#fff', overflowX: 'hidden' }}>
            ...
            {/* ══════ MODAL ══════ */}
            {modal && (
                <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, backdropFilter:'blur(6px)', padding:'16px' }}
                    ...
                    <div style={{ background:'#fff', borderRadius:'22px', padding:'clamp(20px,4vw,36px)', width:'100%', maxWidth: modal === 'signup' ? '480px' : '420px', maxHeight:'92vh', overflowY:'auto', boxShadow:'0 32px 80px rgba(0,0,0,.25)', animation:'modalIn .25s ease', position:'relative' }}>
                        ...
                        {modal === 'login'
                            ? <LoginModal onClose={() => setModal(null)} onSwitchToSignup={handleSwitchToSignup} prefillEmail={loginPrefillEmail}/>
                            : <SignupModal onClose={() => setModal(null)} onSwitchToLogin={handleSwitchToLogin}/>
                        }
                    </div>
                </div>
            )}
