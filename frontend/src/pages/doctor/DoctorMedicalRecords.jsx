    ...
    return (
        <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#f0f4f8', fontFamily:"'DM Sans','Outfit',sans-serif" }}>
            <style>{`
                ...
                .rec-card { transition:box-shadow .15s,transform .15s; }
                .rec-card:hover { box-shadow:0 8px 24px rgba(0,0,0,.1)!important; transform:translateY(-2px); cursor:pointer; }
            `}</style>

            {/* Hero */}
            <div style={{ background:'linear-gradient(135deg,#0f3460,#185FA5)', padding:'20px 28px 24px', flexShrink:0 }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    <button onClick={() => navigate('/doctor/dashboard')}
                            style={{ padding:'8px 16px', borderRadius:'9px', border:'1px solid rgba(255,255,255,.25)', background:'rgba(255,255,255,.12)', color:'#fff', fontSize:'12px', fontWeight:600, cursor:'pointer' }}>
                        ← Dashboard
                    </button>
                </div>
            </div>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:'20px 28px' }}>
                ...
                {loading ? (
                    ...
                    <div style={{ background:'#fff', borderRadius:'16px', border:'1px solid #e8edf2', padding:'70px 20px', textAlign:'center' }}>
                        ...
                        <div style={{ fontSize:'12px', color:'#94a3b8' }}>Records are created from appointment actions</div>
                    </div>
                ) : (
                        ...
                        {filtered.map((rec, idx) => {
                            ...
                            return (
                                <div
                                    key={rec.id}
                                    className="rec-card"
                                    onClick={() => navigate(`/doctor/medical-records/${rec.id}`)}
                                    style={{ background:'#fff', borderRadius:'16px', border:'1px solid #e8edf2', boxShadow:'0 1px 4px rgba(0,0,0,.04)', padding:'18px' }}>

                                    <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'14px' }}>
                                        <div style={{ width:'40px', height:'40px', borderRadius:'12px', background:bg, color:tx, fontSize:'13px', fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{ini}</div>
                                        <div>
                                            <div style={{ fontSize:'13px', fontWeight:700, color:'#0f172a' }}>{rec.patientName || 'Unknown'}</div>
                                            <div style={{ fontSize:'10px', color:'#94a3b8' }}>Appt #{rec.appointmentId} · {dateStr}</div>
                                        </div>
                                    </div>

                                    <div style={{ background:'#f8fafc', borderRadius:'10px', padding:'10px 12px', marginBottom:'10px' }}>
                                        <div style={{ fontSize:'9px', fontWeight:700, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:'4px' }}>Diagnosis</div>
                                        <div style={{ fontSize:'12px', color:'#374151', fontWeight:600 }}>{rec.diagnosis || '—'}</div>
                                    </div>

                                    {rec.notes && (
                                        <div style={{ fontSize:'11px', color:'#6b7280', lineHeight:1.6, marginBottom:'12px', overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' }}>
                                            {rec.notes}
                                        </div>
                                    )}
                                    ...
                                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                                        ...
                                        <span style={{ fontSize:'10px', color:'#94a3b8' }}>MR-{String(rec.id).padStart(4,'0')}</span>
                                    </div>
                                </div>
                            );
                        })}
