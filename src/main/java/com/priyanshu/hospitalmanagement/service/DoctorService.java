...
/**
 * Doctor management — onboarding, availability, profile.
 *
 * Welcome email on onboarding is dispatched via {@link NotificationDispatcher}.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class DoctorService {
    ...
    private final DepartmentRepository departmentRepository;
    private final PasswordEncoder passwordEncoder;
    private final NotificationDispatcher notificationDispatcher;   // ← replaces EmailService
    ...
    @Transactional(readOnly = true)
    public Page<DoctorResponseDto> getAllDoctors(int page, int size) {
        return doctorRepository.findAll(PageRequest.of(page, size)).map(this::mapToDto);
    }
    ...
    public DoctorResponseDto onBoardNewDoctor(OnboardDoctorRequestDto dto) {
        ...
        Doctor saved = doctorRepository.save(doctor);
        log.info("Doctor onboarded: {} (id={})", saved.getName(), saved.getId());

        // Welcome email — fault-tolerant, Kafka or direct based on kafka.enabled
        notificationDispatcher.sendDoctorWelcome(
                saved.getEmail(),
                saved.getName(),
                dto.getPassword()   // plain text — one-time, they should reset
        );
    ...
    // =========================================================================
    @Transactional(readOnly = true)
    public Page<DoctorResponseDto> searchDoctors(String name, String specialization,
                                                 int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
    ...
    private DoctorResponseDto mapToDto(Doctor d) {
        ...
        dto.setBio(d.getBio());
        dto.setProfileImageUrl(d.getProfileImageUrl());
        dto.setDepartments(d.getDepartments().stream()
                .map(Department::getName).collect(Collectors.toSet()));
        return dto;
    }
}
