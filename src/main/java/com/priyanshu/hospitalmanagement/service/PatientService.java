 ...
 *
 * The welcome email on profile creation is dispatched via
 * {@link NotificationDispatcher} — Kafka or direct, fault-tolerant.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class PatientService {
    ...
    public PatientResponseDto createPatientProfile(CreatePatientProfileRequestDto dto) {
        ...
        Patient saved = patientRepository.save(patient);
        log.info("Patient profile created for: {}", username);

        notificationDispatcher.sendPatientWelcome(user.getUsername(), dto.getName());
